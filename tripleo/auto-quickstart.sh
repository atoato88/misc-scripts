...
if [[ -n ${CONFIG} ]]
then
  PARAM_CONFIG0="-c .quickstart/${CONFIG}" 
...
bash quickstart.sh -R master --tags all ${PARAM_CONFIG0} -p quickstart.yml 127.0.0.2
if [[ $? -gt 0 ]]
then
  echo NG retry
...
bash ./tripleo-quickstart/quickstart.sh -R master --no-clone --tags all ${PARAM_NODE} ${PARAM_CONFIG} -I --teardown none -p quickstart-extras-undercloud.yml 127.0.0.2
...
bash ./tripleo-quickstart/quickstart.sh -R master --no-clone --tags all ${PARAM_NODE} ${PARAM_CONFIG} -I --teardown none -p quickstart-extras-overcloud-prep.yml 127.0.0.2
if [[ $? -gt 0 ]]
then
  echo NG retry
  bash ./tripleo-quickstart/quickstart.sh -R master --no-clone --tags all ${PARAM_NODE} ${PARAM_CONFIG} -I --teardown none -p quickstart-extras-overcloud-prep.yml 127.0.0.2
...
bash ./tripleo-quickstart/quickstart.sh -R master --no-clone --tags all ${PARAM_NODE} ${PARAM_CONFIG} -I --teardown none -p quickstart-extras-overcloud.yml 127.0.0.2

print_separation "overcloud validation"
bash ./tripleo-quickstart/quickstart.sh -R master --no-clone --tags all ${PARAM_NODE} ${PARAM_CONFIG} -I --teardown none -p quickstart-extras-validate.yml 127.0.0.2
